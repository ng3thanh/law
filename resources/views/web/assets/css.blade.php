<!-- Bootstrap CSS-->
<link rel="stylesheet" href="{{ asset('web/vendor/bootstrap/css/bootstrap.min.css') }}">
<!-- Font Awesome CSS-->
<link rel="stylesheet" href="{{ asset('web/vendor/font-awesome/css/font-awesome.min.css') }}">
<!-- Custom icon font-->
<link rel="stylesheet" href="{{ asset('web/css/fontastic.css') }}">
<!-- Google fonts - Open Sans-->
<link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Open+Sans:300,400,700">
<!-- Fancybox-->
<link rel="stylesheet" href="{{ asset('web/vendor/@fancyapps/fancybox/jquery.fancybox.min.css') }}">
<!-- theme stylesheet-->
<link rel="stylesheet" href="{{ asset('web/css/style.default.css') }}">
<!-- Favicon-->
<link rel="shortcut icon" href="{{ asset('web/img/favicon.png') }}">
<link rel="shortcut icon" href="{{ asset('web/img/nova_favicon.ico') }}">
