...
var ajaxCommon = function () {
    ...
    var controlProgress = function controlProgress(isShow) {
        if (isShow) {
            progressBlock.show();
            progressBar.show();
        } else {
    ...
    var getSorted = function getSorted(el) {
        var sorted = {};
    ...
    var setSorted = function setSorted(sorted, controlEl) {
    ...
    var getPage = function getPage(el) {
        var page = 0;
        if ($(el).attr('data-cur-page')) page = parseInt($(el).attr('data-cur-page'));
    ...
    var createMessageBlock = function createMessageBlock(msgState, msgContent) {
        var html = '';
        if (msgState == 'warning') {
    ...
    var getPostAndSortingAndPaging = function getPostAndSortingAndPaging(url, page, sorted, controlEl) {
        var curPage = 0;
        var methodRequest = 'get';
        var urlSearch = url;

        if (typeof page != "undefined" && page != null) curPage = parseInt(page);

        if (curPage > 0) urlSearch = urlSearch + '?page=' + curPage;

        if (controlEl.hasOwnProperty('formId') && controlEl.formId != '') methodRequest = $(controlEl.formId).attr('method');
        ...
        $.ajax({
            ...
            error: function error(err) {
                createMessageBlock('error', 'Please try again');
            }
        });
    };

    var exportFile = function exportFile(url, formRequest) {
        var methodRequest = 'get';

        if (formRequest != null) methodRequest = $(formRequest).attr('method');
    ...
    var search = function search(url, controlEl) {
        var sorted = {};
        var page = 0;
    ...
    var exportCsv = function exportCsv(url, controlEl) {
        var formRequest = null;
        if (controlEl.hasOwnProperty('formId') && controlEl.formId != '') formRequest = controlEl.formId;
